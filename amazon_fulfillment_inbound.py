...
def load_all_shipments(marketplace='US', **kwargs):
    """
...
@throttle_retry()
@load_all_pages(extras=dict(QueryType='NEXT_TOKEN'))
def load_all_shipment_items(marketplace='US', **kwargs):
    """
    a generator function to return all pages, obtained by NextToken
    """
    response = FulfillmentInbound(
...
def load_all_inbound_plans(marketplace='US', **kwargs):
    """
    a generator function to return all pages, obtained by NextToken
    """
    response = FulfillmentInbound(
...
def get_all_shipments(marketplaces=['US', 'UK'], **kwargs):
    """
    ...
    for marketplace in to_list(marketplaces):
        logger.info(f'Retrieving Fulfillment Inbound Shipments from {marketplace} \n {kwargs}')
        response = load_all_shipments(marketplace, **kwargs)
...
def get_all_shipment_items(marketplaces=['US', 'UK'], **kwargs):
    """
    ...
    for marketplace in to_list(marketplaces):
        logger.info(f'Retrieving Fulfillment Inbound Shipment Items from {marketplace} \n {kwargs}')
        response = load_all_shipment_items(marketplace, **kwargs)
...
def get_all_inbound_plans(marketplaces=['US', 'UK'], statuses=['ACTIVE', 'VOIDED', 'SHIPPED'], **kwargs):
    """
    ...
    for marketplace in to_list(marketplaces):
        ...
        for status in to_list(statuses):
            logger.info(f'Retrieving Inbound Plans ({status}) from {marketplace} \n {kwargs}')

            response = load_all_inbound_plans(marketplace, status=status, **kwargs)
...
def get_all_inbound_plans_info(marketplaces=['US', 'UK'], inbound_plan_ids='All', last_updated_at='1900-01-01'):
    """
    ...
    for marketplace in to_list(marketplaces):
        logger.info(f"Fetching top level information of inbound plans from {marketplace}")
...
def get_all_inbound_plans_shipments(inbound_plan_ids='All', last_updated_at='01-01-1970'):
    """
...
def update_data(table_name, start_date=None, end_date=None, marketplaces=['US', 'UK']):
    """
